    ...
    private WebDriver webDriverSetup(){
        ...
        if (browserName.toLowerCase().contains("firefox")) {
        	...
        	if (ConfigurationManager.getInstance().getSafariUseOptions()!= null && ConfigurationManager.getInstance().getSafariUseOptions()) {
	        	...
	        	if (ConfigurationManager.getInstance().getSafariDataDirectory().length() > 0)
	        		options.setDataDir(new File(ConfigurationManager.getInstance().getSafariDataDirectory()));
	        	if (ConfigurationManager.getInstance().getSafariDriverExtension().length() > 0)
	        	...
	        	if (ConfigurationManager.getInstance().getSafariSkipExtentionInstallation() != null)
